...
Проект построен по модульному принципу, разделяя логику интерфейса, обработки данных и взаимодействия с базой данных. Это облегчает поддержку и масштабирование кода.
...
Программа условно делится на 4 слоя:

---

#### 1. Интерфейс пользователя (GUI) — `views/`

- Отвечает за визуальное взаимодействие с пользователем.
- Реализован с помощью **Tkinter**, с вкладками по назначению:
  - Загрузка объектных и локальных смет
  - Анализ данных
  - Управление базой данных

---

#### 2. Логика обработки данных — `parsing/` и `reports/`

- `parsing/`: преобразование сметных файлов различных форматов (XML, XLS, XLSX, GGE) в структуру, пригодную для сохранения в базу данных.
- `reports/`: выполнение анализа и генерация Excel-отчетов на основе данных из базы.

---

#### 3. Работа с базой данных — `models/`

- Используется **SQLAlchemy** как ORM для взаимодействия с PostgreSQL.
- Модели описывают таблицы и их связи.
- Утилиты в папке `models/` помогают обрабатывать и извлекать данные.
...
- База данных содержит следующие таблицы:
  - `objects`, `object_estimates`, `local_estimates`, `sections`, `work`, `materials`
- Все таблицы связаны между собой внешними ключами.
- Для быстрого развёртывания структуры и наполнения можно использовать SQL-дампы из папки `dump_database/`.
...
Показывает, сколько раз встречаются определённые работы и материалы во всех сметах. Используется сортировка по общему количеству вхождений.

![Количество вхождений](img/frequency_report.png)

---

#### Количество смет

Отражает, в скольких различных сметах встречаются те или иные работы или материалы. Сортировка по числу уникальных смет.

![Количество смет](img/unique_smeta_count.png)

---

#### Удельная стоимость

Анализ работ и материалов по удельной стоимости — рассчитывается как доля от стоимости по сравнению с полной стоимостью сметы.

![Удельная стоимость](img/unit_price_report.png)

---

#### АР/КР (Архитектурно-конструктивный анализ)

Рассчитывает процент работ, относящихся к архитектурным и конструктивным элементам (по кодам), в общей стоимости объекта.

![АР/КР анализ](img/arkr_analysis.png)

---

Все отчёты сохраняются в формате `.xlsx` и автоматически открываются после генерации.
